(() => {
  ...
  function setComponentPropertiesFromObservedAttributes(component, observedAttributes, onUpdate) {
    const properties = [];
    observedAttributes.forEach((prop) => {
      prop = prop.trim();
      if (!directives.has(prop) && !(prop.startsWith("data-") || prop === "class" || prop === "style")) {
        let value = component.getAttribute(prop) ?? "";
        ...
        if (boolean_attributes_default.hasOwnProperty(prop)) {
          value = boolean_attributes_default[prop].value;
  ...
  // src/enums/ShadowRootModeExtended.enum.ts
  var ShadowRootModeExtended = /* @__PURE__ */ ((ShadowRootModeExtended2) => {
    ShadowRootModeExtended2["OPEN"] = "open";
    ShadowRootModeExtended2["CLOSED"] = "closed";
    ShadowRootModeExtended2["NONE"] = "none";
    return ShadowRootModeExtended2;
  })(ShadowRootModeExtended || {});
 ...
 return this.${fn}`);
      ...
      if (directiveNode === this.node) {
        ...
        for (let { name, value, executables } of this.attributes) {
          if (executables.length) {
            ...
            if (this.node[camelName] !== void 0) {
              newValue = jsonParse(newValue);
              if (newValue !== this.node[camelName]) {
                this.node[camelName] = newValue;
              }
  ...
  function trackNode(node, component, opt) {
    ...
    if (nodeName === "SLOT") {
      if (customSlot) {
        return renderCustomSlot(node, customSlotChildNodes).forEach((child) => trackNode(child, component, opt));
      }
  ...
  var WebComponent = class extends HTMLElement {
    constructor() {
      ...
      this.$properties.push(...setComponentPropertiesFromObservedAttributes(this, observedAttributes, (prop, oldValue, newValue) => {
        this.forceUpdate();
        if (this.mounted) {
    ...
    get $context() {
      return Object.setPrototypeOf({
        ...$.get(this).contextSource?.$context,
        ...$.get(this).$context
      }, {
    ...
    updateContext(ctx) {
      ...
      if (this.mounted) {
        this.forceUpdate();
        $.get(this).contextSubscribers.forEach((cb) => cb($.get(this).$context));
      }
    }
    connectedCallback() {
      ...
      try {
        ...
        if ($.get(this).contextSource) {
          $.get(this).unsubscribeCtx = $.get(this).contextSource.$context.subscribe((newContext) => {
            this.forceUpdate();
            if (this.mounted) {
              this.onUpdate("$context", $.get(this).$context, newContext);
            }
            $.get(this).contextSubscribers.forEach((cb) => cb(newContext));
          });
        }
        if (parsed) {
          this.forceUpdate();
        } else {
          this.$properties.push(...setupComponentPropertiesForAutoUpdate(this, (prop, oldValue, newValue) => {
            this.forceUpdate();
            if (this.mounted) {
              this.onUpdate(prop, oldValue, newValue);
            }
    ...
    attributeChangedCallback(name, oldValue, newValue) {
      try {
        if (!(name.startsWith("data-") || name === "class" || name === "style")) {
          const prop = turnKebabToCamelCasing(name);
  ...
  function getClosestWebComponentAncestor(component) {
  ...
  ContextProviderComponent.mode = ShadowRootModeExtended.NONE;
