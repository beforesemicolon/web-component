  ...
  function setComponentPropertiesFromObservedAttributes(comp, attrs, attrsMap = {}, cb) {
    ...
    attrs.forEach((attr) => {
      ...
      if (!directives.has(attr) && !(attr.startsWith("data-") || attr === "class" || attr === "style")) {
        let value = comp.getAttribute(attr) ?? "";
 ...
 return this.${fn}`);
  ...
  function trackNode(node, component, opt) {
    ...
    if (nodeName === "SLOT") {
      ...
      if (customSlot) {
        return renderCustomSlot(node, customSlotChildNodes, attrs).forEach((child) => trackNode(child, component, opt));
      }
      renderSlot(node, attrs, (nodes) => {
        nodes.forEach((n) => {
  ...
  var WebComponent = class extends HTMLElement {
    constructor() {
      ...
      this.$properties.push(...setComponentPropertiesFromObservedAttributes(this, observedAttributes, meta.attrPropsMap, (prop, oldValue, newValue) => {
        this.forceUpdate();
        if (this.mounted) {
    ...
    connectedCallback() {
      ...
      const { initialContext } = this.constructor;
