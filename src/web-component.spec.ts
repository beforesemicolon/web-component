...
describe('WebComponent', () => {
	...
	describe('hashed attributes', () => {
		class MyItem extends WebComponent {
		...
		class CustomAttr extends WebComponent {
			static initialContext = {
				x: 200
			}
			static observedAttributes = ['cls', 'value', 'style'];
			count = 3;

			get template() {
				return '<div class="{cls}">\n' +
					'<div #if="value === \'sample\'">{value}</div>\n' +
					'<div #repeat="2">{value}</div>\n' +
					'<p style="{this.style.cssText}" #ref="paragraph">{value}</p>\n' +
					// '<div #repeat="2">last</div>\n' +
					// '<div #repeat="2">after</div>\n' +
					// '<my-item #repeat="count"></my-item>\n' +
					'</div>'
			}
		}
		...
		MyItem.register();
		CustomAttr.register();
		const ca = new CustomAttr();

		document.body.appendChild(ca);

		// @ts-ignore
		const item = ca.root?.querySelector('my-item') as WebComponent;

		it('should handle hashed attributes on mount', () => {
			// expect(ca.refs.paragraph.outerHTML).toBe('<p style=""></p>');
			// expect(ca.root?.innerHTML).toBe('<div class="">\n' +
			// 	'<!--#if: value === \'sample\'-->\n' +
			// 	'<p style=""></p>\n' +
			// 	'</div>');
			...
			// 	'<p style="">sample</p>\n' +
			// 	'<div>sample</div>\n' +
			// 	'<!--#repeat: 3--><my-item></my-item><my-item></my-item><my-item></my-item>\n' +
			// 	'</div>');
		});
	});
});