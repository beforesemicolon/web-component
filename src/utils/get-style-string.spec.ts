...
describe('getStyleString', () => {
	...
	it('should return css inside a style tag id by the tag name', () => {
		expect(getStyleString(style, 'test-tag')).toEqual('<style id="test-tag">:host { background: red; } :host ( .active ) { background: green; }</style>')
	});

	it('should replace :host with tag name if no shadow root', () => {
		expect(getStyleString(style, 'test-tag', false)).toEqual('<style id="test-tag">test-tag { background: red; } test-tag.active { background: green; }</style>')
	});
});